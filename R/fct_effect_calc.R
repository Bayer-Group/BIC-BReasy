effect_calc <- function(data = data, effect = effect, param = param, outcome = outcome, scope = scope, datascope = datascope, population = population, treatment = treatment, verum = verum, comparator = comparator, 
  ...
  for (j in 1:length(outcome)) {
    ...
    if(strat != "Overall") { 
      ...
      result_test_excess <- c(excess, nnt)
      names(result_test_excess) <- c("Excess","nnt")
      ...
      if (effect == "excess") {
        ...
        names(result_test_1[[j]]) <- c("ESTIMATE","ANALYSIS_SET","OUTCOME","DATA_SCOPE","SUBGROUP","SUBLEVEL","STRATVAR","STRATUM","NUMBER_EVENTS_VERUM","NUMBER_PATIENTS_VERUM","NUMBER_EVENTS_COMP","NUMBER_PATIENTS_COMP","EFFECT_EXCESS","NNT")
      }
    }
      ...
      if (effect == "excess") {
        result_test_1[[j]] <- c("Incidence Rate by 100 pat-yrs",population,outcome[[j]],datascope,"Overall","All",x1,n1,x2,n2,excess,nnt)
        names(result_test_1[[j]]) <- c("ESTIMATE","ANALYSIS_SET","OUTCOME","DATA_SCOPE","SUBGROUP","SUBLEVEL","NUMBER_EVENTS_VERUM","NUMBER_PATIENTS_VERUM","NUMBER_EVENTS_COMP","NUMBER_PATIENTS_COMP","EFFECT_EXCESS","NNT")
      }
    }
  }
