      ...
      for (i in 1:length(PARAM_levels)) {
        for (j in 1:length(AVISITN_levels)) {
          if (dim(adtte_data2()[which(adtte_data2()[[input$parameter]]==PARAM_levels[i] & adtte_data2()[[input$visit]]==AVISITN_levels[j]),])[1] != 0) {
            ...
            csv2[which(csv2$OUTCOME==PARAM_levels[i] & csv2$AVISITN == AVISITN_levels[j]),]$NUMBER_PATIENTS_VERUM <- dim(adtte_data2()[which(adtte_data2()[[input$parameter]]==PARAM_levels[i] & adtte_data2()[[input$visit]]==AVISITN_levels[j] & adtte_data2()[,which(names(adtte_data2())==Sel_trt)] == input$verum),])[1]
            csv2[which(csv2$OUTCOME==PARAM_levels[i] & csv2$AVISITN == AVISITN_levels[j]),]$NUMBER_PATIENTS_COMP <- dim(adtte_data2()[which(adtte_data2()[[input$parameter]]==PARAM_levels[i] & adtte_data2()[[input$visit]]==AVISITN_levels[j] & adtte_data2()[,which(names(adtte_data2())==Sel_trt)] == input$compa),])[1]
